...
- **Scenario:** A compromised/retagged release of any of these actions executes inside `publish.yml` where `secrets.DOCKERHUB_TOKEN` is live (publish.yml:78) — the classic `tj-actions/changed-files` attack. The attacker exfiltrates the token and pushes a malicious `securedbytyler/scrye:latest`. For a security-scanner product this is the worst-case supply-chain outcome.
...
- **Scenario:** External PR merged into `dev` → publish run fails at login → `securedbytyler/scrye:dev` goes stale while everyone believes it mirrors `dev`.
...
- **INF-18:** `README.md:205` quick start only builds locally; the published `securedbytyler/scrye` image is documented only in CONTRIBUTING. (See also DOC-1.)
...
- **Fix:** Rewrite Quick start/Building/Roadmap to reference `docker pull securedbytyler/scrye:…`.
