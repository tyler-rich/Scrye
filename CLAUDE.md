...
- **All commits and PRs are authored as the user, not as Claude.** Configure the local git identity
  ...
  decided 2026-08-09, declined rather than deferred.** Web-UI/API merges are therefore authored
  under that real name, and those commits (including promotion merge commits) are correct as they
  stand — do not flag them as drift or a violation, and do not re-raise the option of renaming the
  profile to `tyler-rich`. The invariant this rule protects is that **no Claude/Anthropic identity
  appears in authorship**; the maintainer's real name satisfies it, and the accepted cost — two
  author strings for the same person in history — is cosmetic. (A merge commit leaves the
  individual promoted commits' own authorship intact, so only the merge commit itself is affected.
  See `docs/ARCHIVE.md` § Deviations, 2026-07-13 squash-merge authorship and the 2026-08-09
...
8. Commits are **verified** — not assumed — to be under the user's git identity, checked via
   ...
   for what you write (never a footer) and for the `gh`-only rule that keeps authorship correct.
