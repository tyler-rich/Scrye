...
4. Open the PR against `dev` (not `main` — see § Branching model above) with a clear summary of
   ...
   **leave it** — do not `PATCH` the posted body to remove it. Such an edit runs
   under a bot identity and leaves a permanent "claude (Bot)" entry in the PR's
   public edit history, while the footer gets re-appended regardless. Say so in
   your summary instead. See `CLAUDE.md` § Attribution for the full reasoning.
