...
4. Open the PR against `dev` (not `main` — see § Branching model above) with a clear summary of
   ...
   footers** — no "Generated by …" line, co-author trailer, or session link.
   Compose them without one; verify authorship with
   `git log --format="%an <%ae>%n%B"`. If your tooling appends a footer anyway,
   re-read the posted body and issue **one** `PATCH` to remove it, then re-read
   once to confirm. **If the footer comes back, stop** — do not attempt a second
   strip on that body; say "strip attempted, did not hold" in your summary
   instead. The one-attempt cap is deliberate: each `PATCH` runs under a bot
   identity and leaves a permanent "claude (Bot)" entry in the PR's public edit
   history, and a retry has never changed the result. See `CLAUDE.md`
   § Attribution for the full reasoning.
